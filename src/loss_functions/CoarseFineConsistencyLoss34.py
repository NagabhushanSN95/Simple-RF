...
class CoarseFineConsistencyLoss(LossFunctionParent):
    def __init__(self, configs: dict, loss_configs: dict) -> None:
    ...
    def compute_loss(self, input_dict: dict, output_dict: dict, model, return_loss_maps: bool = False) -> dict:
        total_loss = torch.tensor(0).to(input_dict['target_rgb'])

        # Loss requires both coarse and fine models
        if not self.coarse_model_needed or not self.fine_model_needed:
        ...
        loss = self.compute_depth_loss(depths_coarse, depths_fine, indices_mask_nerf, indices_mask_sd, rays_o, rays_d, extrinsics, gt_images, pixel_ids, intrinsics, resolution, return_loss_maps)
    ...
    def compute_depth_loss(self, depth_coarse, depth_fine, indices_mask_nerf, indices_mask_sd,
        ...
        loss_nerf, loss_map_nerf_coarse, loss_map_nerf_fine = self.compute_loss_nerf(depth_coarse, depth_fine, indices_mask_nerf,
                                                                                     rays_o, rays_d, extrinsics, gt_images, pixel_ids,
    ...
    def compute_loss_nerf(self, depth1, depth2, indices_mask_nerf, rays_o, rays_d, extrinsics, gt_images,
        ...
        poses_b = extrinsics[image_ids_b]
    ...
    @classmethod
    def compute_loss_sd(cls, depths_coarse: Tensor, depths_fine: Tensor, indices_mask_sd: Tensor)\
            -> tuple[Tensor, Tensor]:
        """
        ...
        loss, loss_map = cls.compute_depth_mse(depths_coarse, depths_fine.detach())
