...
class DepthMAE:
    ...
    def compute_avg_mae(self, old_data: pandas.DataFrame, gt_depth_dirpath: Path, pred_videos_dirpath: Path,
        ...
        :param old_data:
        :param gt_depth_dirpath: Should be path to databases/MipNeRF360/data
        ...
        for i, frame_data in tqdm(self.frames_data.iterrows(), total=self.frames_data.shape[0], leave=self.verbose_log):
            ...
            if old_data is not None and old_data.loc[
                ...
                continue
            resolution_suffix = self.get_resolution_suffix(resolution_suffix, scene_name)
            ...
            qa_score = self.compute_depth_mae(gt_depth, pred_depth)
...
def start_qa(pred_videos_dirpath: Path, gt_depth_dirpath: Path, frames_datapath: Path, pred_depths_dirname: str, 
    ...
    mse_computer = DepthMAE(frames_data)
    avg_mae, mae_data = mse_computer.compute_avg_mae(mae_data, gt_depth_dirpath, pred_videos_dirpath,
...
def demo1():
    ...
    downsampling_factor = 1
    avg_mae = start_qa(pred_videos_dirpath, gt_depth_dirpath, frames_data_path, pred_depths_dirname, resolution_suffix,
                       downsampling_factor)
    return avg_mae
...
def demo2(args: dict):
    ...
    avg_mae = start_qa(pred_videos_dirpath, gt_depth_dirpath, frames_datapath, pred_depths_dirname, resolution_suffix,
                        downsampling_factor)
    return avg_mae
