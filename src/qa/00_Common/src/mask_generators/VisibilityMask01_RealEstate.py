...
def start_generation(gen_configs: dict):
    ...
    scene_nums = numpy.unique(train_video_data['scene_num'].to_numpy())

    mask_computer = VisibilityMaskComputer(gen_configs)

    for scene_num in tqdm(scene_nums):

        train_frame_nums = train_video_data.loc[train_video_data['scene_num'] == scene_num]['pred_frame_num'].to_numpy()
        test_frame_nums = test_video_data.loc[test_video_data['scene_num'] == scene_num]['pred_frame_num'].to_numpy()
        ...
        model_configs_path = dense_nerf_dirpath / f'{scene_num:05}/ModelConfigs.json'
        ...
        for train_frame_num in train_frame_nums:
            for test_frame_num in test_frame_nums:
                mask_output_path = output_dirpath / f'{scene_num:05}/visibility_masks/{test_frame_num:04}_{train_frame_num:04}.npy'
                if mask_output_path.exists():
                    continue
        
                train_frame_path = database_dirpath / f'test/database_data/{scene_num:05}/rgb/{train_frame_num:04}.png'
                # test_frame_path = database_dirpath / f'test/database_data/{scene_num:05}/rgb{suffix}/{test_frame_num:04}.png'
                train_depth_path = depth_dirpath / f'{scene_num:05}/predicted_depths/{train_frame_num:04}.npy'
                test_depth_path = depth_dirpath / f'{scene_num:05}/predicted_depths/{test_frame_num:04}.npy'
