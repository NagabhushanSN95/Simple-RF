...
class DepthMAE:
    ...
    @staticmethod
    def compute_depth_mae(gt_depth: numpy.ndarray, eval_depth: numpy.ndarray, mask: numpy.ndarray):
        ...
        mae = numpy.sum(numpy.abs(mask * error)) / numpy.sum(mask)
        return mae

    def compute_avg_mae(self, old_data: pandas.DataFrame, gt_depth_dirpath: Path, pred_videos_dirpath: Path,
                        database_dirpath: Path, pred_depths_dirname: str, resolution_suffix: str, downsampling_factor: int, mask_folder_name: str):
        """
        ...
        for i, frame_data in tqdm(self.test_frames_data.iterrows(), total=self.test_frames_data.shape[0], leave=self.verbose_log):
            ...
            qa_score = self.compute_depth_mae(gt_depth, pred_depth, mask)
        ...
        if isinstance(avg_mae, numpy.ndarray):
...
def start_qa(pred_videos_dirpath: Path, gt_depth_dirpath: Path, database_dirpath: Path, frames_datapath: Path,
             pred_depths_dirname: str, resolution_suffix, downsampling_factor: int, mask_folder_name: str):
    ...
    mae_data_path = pred_videos_dirpath / f'QA_Scores/{pred_depths_dirname}/{this_metric_name}_FrameWise.csv'
    ...
    if pred_depths_dirname in qa_scores:
        if this_metric_name in qa_scores[pred_depths_dirname]:
            avg_mae = qa_scores[pred_depths_dirname][this_metric_name]
            print(f'Average {this_metric_name}: {pred_videos_dirpath.as_posix()} - {pred_depths_dirname}: {avg_mae}')
    ...
    mae_computer = DepthMAE(train_frames_data, test_frames_data)
    avg_mae, mae_data = mae_computer.compute_avg_mae(mae_data, gt_depth_dirpath, pred_videos_dirpath, database_dirpath,
                                                     pred_depths_dirname, resolution_suffix, downsampling_factor,
                                                     mask_folder_name)
    if numpy.isfinite(avg_mae):
        qa_scores[pred_depths_dirname][this_metric_name] = avg_mae
        print(f'Average {this_metric_name}: {pred_videos_dirpath.as_posix()} - {pred_depths_dirname}: {avg_mae}')
...
def demo1():
    ...
    avg_mae = start_qa(pred_videos_dirpath, gt_depth_dirpath, database_dirpath, frames_data_path, pred_depths_dirname, resolution_suffix,
...
def demo2(args: dict):
    ...
    avg_mae = start_qa(pred_videos_dirpath, gt_depth_dirpath, database_dirpath, frames_datapath, pred_depths_dirname,
                       resolution_suffix, downsampling_factor, mask_folder_name)
    return avg_mae
