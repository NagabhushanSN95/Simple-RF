...
# Masked PSNR measure between predicted frames and ground truth frames
...
class PSNR:
    ...
    @staticmethod
    def compute_frame_psnr(gt_frame: numpy.ndarray, eval_frame: numpy.ndarray, mask: numpy.ndarray):
        error = gt_frame.astype('float') - eval_frame.astype('float')
        mask_3d = numpy.stack([mask] * 3, axis=2)
        mse = numpy.sum(numpy.square(mask_3d * error)) / numpy.sum(mask_3d)
    ...
    def compute_avg_psnr(self, old_data: pandas.DataFrame, database_dirpath: Path, pred_videos_dirpath: Path,
        ...
        for i, frame_data in tqdm(self.test_frames_data.iterrows(), total=self.test_frames_data.shape[0], leave=self.verbose_log):
            ...
            qa_score = self.compute_frame_psnr(gt_frame, pred_frame, mask)
...
def start_qa(pred_videos_dirpath: Path, database_dirpath: Path, frames_datapath: Path, pred_frames_dirname: str,
    ...
    psnr_computer = PSNR(train_frames_data, test_frames_data)
    avg_psnr, psnr_data = psnr_computer.compute_avg_psnr(psnr_data, database_dirpath, pred_videos_dirpath,
...
def demo1():
    ...
    avg_psnr = start_qa(pred_videos_dirpath, database_dirpath, frames_data_path, pred_frames_dirname, resolution_suffix,
...
def demo2(args: dict):
    ...
    avg_psnr = start_qa(pred_videos_dirpath, database_dirpath, frames_datapath, pred_frames_dirname, resolution_suffix,
                        downsampling_factor, mask_folder_name)
    return avg_psnr
