...
# Masked SROCC measure between predicted depths and depths from dense input NeRF
...
class DepthSROCC:
    ...
    @staticmethod
    def compute_depth_srocc(gt_depth: numpy.ndarray, eval_depth: numpy.ndarray, mask: numpy.ndarray):
        masked_gt_depth = gt_depth[mask].astype('float')
        masked_eval_depth = eval_depth[mask].astype('float')
        srocc = spearmanr(masked_gt_depth, masked_eval_depth).correlation
        return srocc
    ...
    def compute_avg_srocc(self, old_data: pandas.DataFrame, gt_depth_dirpath: Path, pred_videos_dirpath: Path,
        ...
        for i, frame_data in tqdm(self.test_frames_data.iterrows(), total=self.test_frames_data.shape[0], leave=self.verbose_log):
            ...
            qa_score = self.compute_depth_srocc(gt_depth, pred_depth, mask)
...
def start_qa(pred_videos_dirpath: Path, gt_depth_dirpath: Path, database_dirpath: Path, frames_datapath: Path,
    ...
    srocc_computer = DepthSROCC(train_frames_data, test_frames_data)
    avg_srocc, srocc_data = srocc_computer.compute_avg_srocc(srocc_data, gt_depth_dirpath, pred_videos_dirpath, database_dirpath,
...
def demo1():
    ...
    avg_srocc = start_qa(pred_videos_dirpath, gt_depth_dirpath, database_dirpath, frames_data_path, pred_depths_dirname,
...
def demo2(args: dict):
    ...
    avg_srocc = start_qa(pred_videos_dirpath, gt_depth_dirpath, database_dirpath, frames_datapath, pred_depths_dirname,
                         resolution_suffix, downsampling_factor, mask_folder_name)
    return avg_srocc
