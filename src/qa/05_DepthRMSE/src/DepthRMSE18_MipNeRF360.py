...
class DepthRMSE:
    ...
    def compute_avg_rmse(self, old_data: pandas.DataFrame, gt_depth_dirpath: Path, pred_videos_dirpath: Path,
        ...
        for i, frame_data in tqdm(self.frames_data.iterrows(), total=self.frames_data.shape[0], leave=self.verbose_log):
            ...
            qa_score = self.compute_depth_rmse(gt_depth, pred_depth)
...
def start_qa(pred_videos_dirpath: Path, gt_depth_dirpath: Path, frames_datapath: Path, pred_depths_dirname: str, 
    ...
    rmse_computer = DepthRMSE(frames_data)
...
def demo1():
    ...
    downsampling_factor = 1
    avg_rmse = start_qa(pred_videos_dirpath, gt_depth_dirpath, frames_data_path, pred_depths_dirname, resolution_suffix, 
                        downsampling_factor)
    return avg_rmse
...
def demo2(args: dict):
    ...
    avg_rmse = start_qa(pred_videos_dirpath, gt_depth_dirpath, frames_datapath, pred_depths_dirname, resolution_suffix,
                        downsampling_factor)
    return avg_rmse
