...
this_metric_name = this_filename[:-15]
...
class LPIPS:
    ...
    def compute_avg_lpips(self, old_data: pandas.DataFrame, database_dirpath: Path, pred_videos_dirpath: Path,
        ...
        :param old_data:
        :param database_dirpath: Should be path to databases/NeRF_Synthetic/data
        ...
        for i, frame_data in tqdm(self.frames_data.iterrows(), total=self.frames_data.shape[0], leave=self.verbose_log):
            scene_name, group, pred_frame_num = frame_data
            if old_data is not None and old_data.loc[
                (old_data['scene_name'] == scene_name) & (old_data['pred_frame_num'] == pred_frame_num)
            ].size > 0:
                continue
            gt_frame_path = database_dirpath / f'all/database_data/{scene_name}/{group}/rgb/{pred_frame_num:04}.png'
    ...
    def read_image(path: Path):
        ...
        if image.shape[2] == 4:
            rgb = image[:, :, :3].astype('float32') / 255
            alpha = image[:, :, 3:].astype('float32') / 255
...
def start_qa(pred_videos_dirpath: Path, database_dirpath: Path, frames_datapath: Path, pred_frames_dirname: str, downsampling_factor: int):
    ...
    frames_data = pandas.read_csv(frames_datapath)[['scene_name', 'group', 'pred_frame_num']]

    mse_computer = LPIPS(frames_data)
    avg_lpips, lpips_data = mse_computer.compute_avg_lpips(lpips_data, database_dirpath, pred_videos_dirpath, pred_frames_dirname, downsampling_factor)
...
def demo1():
    root_dirpath = Path('../../../../')
    pred_videos_dirpath = root_dirpath / 'runs/testing/test3012'
    ...
    avg_lpips = start_qa(pred_videos_dirpath, database_dirpath, frames_data_path, pred_frames_dirname, downsampling_factor)
    return avg_lpips
...
def demo2(args: dict):
    ...
    avg_lpips = start_qa(pred_videos_dirpath, database_dirpath, frames_datapath, pred_frames_dirname, downsampling_factor)
    return avg_lpips
